...
### main function
### train
def train_prototype_hypersphere(dat_train,pretrained_model,sen_len,pretrained_model_path,save_checkpoint_path,batch_size = 1,train_K = 8,train_Q = 8, train_episodes = 300,random_sample = False,val_episodes = 30,val_steps = 10):
    ...
    if random_sample==False:
        ...
        region_ref = dat_train[dat_train.region == 1]
        nonregion_ref = dat_train.loc[random.sample(dat_train[dat_train.region == 0].index.tolist(),len(region_ref))]
    ...
    support_tls_mean = torch.tensor(np.zeros(shape = (sum(dat_train.region == 1),len(support_region_nonregion[0][0][0][0])))).to("cpu")
    for n in tqdm(range(0,len(support_region_nonregion))):
        tmp = support_region_nonregion[n][0][0][1].to("cpu")
        support_tls_mean[n] = tmp
    support_tls_mean = support_tls_mean[1:].mean(0, keepdim=True)
    
    support_nontls_mean = torch.tensor(np.zeros(shape = (sum(dat_train.region == 0),len(support_region_nonregion[0][0][0][0])))).to("cpu")
    for n in tqdm(range(0,len(support_region_nonregion))):
        tmp = support_region_nonregion[n][0][0][0].to("cpu")
        support_nontls_mean[n] = tmp
