...
#'
#'
#' @return If the data type is sc_st, the sentences of single cells or spots which are saved in the meta.data of seurat object or saved in the single txt file.
#' @export 

generate_sentences <- function(seu_obj,sen_len=260,region_info,save_inseu=TRUE,save_path,genes_representor,data_type,envir_path){
    ...
    if(data_type == "sc_st"){
        ...
        sc_dat_inputdf <- as.data.frame(seu_obj)
        model_used_genes <- data.table::fread(genes_representor) %>%
            as.data.frame()
        model_used_genes <- model_used_genes$Gene
        input_gene = intersect(rownames(sc_dat_inputdf),model_used_genes)
        ...
        if(save_inseu){
            seu_obj <- seu_obj[,gsentence_df$cell_barcode]
            seu_obj["sentence",] <- gsentence_df$sentence
            return(seu_obj)
        }else{
            write.table(gsentence_df,file = paste0(save_path,"_sentence_rank",sen_len,".txt"),quote = F,row.names = F,col.names = T,sep = "\t")
        }
