...
#' @param data_type Default is sc_sc, the other is bulk.
#'
#'
...
run_tlsformer_pred <- function(seu_obj,pretrained_model_path,save_checkpoint_path,envir_path,
                                  pretrained_model = "TLSformer_BERT", sen_len=260,data_type = "sc_sc"){
    ...
    if(data_type=="sc_sc"){
        ...
        dat_pred <- dat_pred[,c("Non-TLS Distance","TLS Distance", "relative_distance","pred_label")]
        colnames(dat_pred) <- c("Non-Target Distance","Target Distance", "relative_distance","pred_label")
